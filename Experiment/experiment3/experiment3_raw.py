...
def classify(history, hour):
	...
	# sd is negative if 0 is stronger than 1
	# sd is positive if 1 is stronger than 0 
	return frequency_class, ratio3, trend, sd
...
def classify_histories(histories, index, hour):
	...
	for hist in histories:
		hist_length = len(hist) / 3
		freqc, ratio, trend, sd = classify(hist, hour)
		fc.append(freqc)
		trends.append(trend)
		ratios.append(ratio)
		sds.append(sd)
	...
	mean_sd = np.mean(sds)
	mean_rat = np.mean(ratios)
	thresh = .5
	neg_thresh = -.5

	if (mean_sd >= thresh):
		trend_final = INCREASING
	elif (mean_sd <= neg_thresh):
		trend_final = DECREASING
	else:
		trend_final = STEADY

	lower_thresh, middle_thresh, upper_thresh = get_freq_thresholds(hist_length)
	# classify frequency
	if (mean_rat <= lower_thresh):
		fc_final = LOW_FREQ
	elif (mean_rat <= middle_thresh):
		fc_final = MID_LOW_FREQ
	elif (mean_rat <= upper_thresh):
		fc_final = MID_HIGH_FREQ
	else:
		fc_final = HIGH_FREQ

	fc_diff = False
	if (fc_final != fc[index]):
		fc_diff = True
	trend_diff = False
	if (trend_final != trends[index]):
		trend_diff = True

	return fc_final, trend_final, fc_diff, trend_diff
...
def runExperiment(datafile, history_length, savename):
	...
	while (entry != ""):
		...
		#	print "i: " + str(i)
			...
			if (firstEntry[i] or justUpdated):
				frequency_class, trend, fc_diff, trend_diff = classify_histories(history, i, int(t_split[0]))

				if (fc_diff):
					num_fc_changes = num_fc_changes + 1
				if (trend_diff):
					num_t_changes = num_t_changes + 1
	...
	fo.write("number times fc is different than intended: " + str(num_fc_changes) + "\n")
	percect_fc = float(num_fc_changes) / float(num_classified)
	fo.write("\tpercent: " + str(percect_fc) + "\n")

	fo.write("number times trend is different than intended: " + str(num_t_changes) + "\n")
	percect_t = float(num_t_changes) / float(num_classified)
...
savename = "raw/hist10_first.txt"
runExperiment(filename, hist_length, savename)

filename = "experiment_data/second_week.txt"
savename = "raw/hist10_second.txt"
runExperiment(filename, hist_length, savename)

filename = "experiment_data/third_week.txt"
savename = "raw/hist10_third.txt"
runExperiment(filename, hist_length, savename)

hist_length = 15

filename = "experiment_data/first_week.txt"
savename = "raw/hist15_first.txt"
runExperiment(filename, hist_length, savename)

filename = "experiment_data/second_week.txt"
savename = "raw/hist15_second.txt"
runExperiment(filename, hist_length, savename)

filename = "experiment_data/third_week.txt"
savename = "raw/hist15_third.txt"
runExperiment(filename, hist_length, savename)

hist_length = 20

filename = "experiment_data/first_week.txt"
savename = "raw/hist20_first.txt"
runExperiment(filename, hist_length, savename)

filename = "experiment_data/second_week.txt"
savename = "raw/hist20_second.txt"
runExperiment(filename, hist_length, savename)

filename = "experiment_data/third_week.txt"
savename = "raw/hist20_third.txt"
runExperiment(filename, hist_length, savename)

hist_length = 30

filename = "experiment_data/first_week.txt"
savename = "raw/hist30_first.txt"
runExperiment(filename, hist_length, savename)

filename = "experiment_data/second_week.txt"
savename = "raw/hist30_second.txt"
runExperiment(filename, hist_length, savename)

filename = "experiment_data/third_week.txt"
savename = "raw/hist30_third.txt"
runExperiment(filename, hist_length, savename)
