...
def runExperiment(datafile, history_length, savename):
	...
	# based on frequency and trend, turn lights on for 5 minutes
		# if they have been OFF for more than a given period of time
	LOW_DEADTIME_THRESH = 120
	MID_LOW_DEADTIME_THRESH = 15
	MID_HIGH_DEADTIME_THRESH = 9
	HIGH_DEADTIME_THRESH = 4
	MIN_ON = 5
	...
	timeSinceActive = [0]*num_sensors
	timeHasBeenActive = [0]*num_sensors
	...
	while (entry != ""):
		...
		#	print "i: " + str(i)
			...
			# if ever active, reset the time since last active
			if (time_left > 0):
				timeSinceActive[i] = 0
				timeHasBeenActive[i] = timeHasBeenActive[i] + 1
			# otherwise, increment the number of minutes it has been since the light was on
			else:
				timeHasBeenActive[i] = 0
				timeSinceActive[i] = timeSinceActive[i] + 1
			...
			if (frequency_class == LOW_FREQ):
				if (trend == INCREASING):
					...
					if (int(entry_array[i]) == 1):
						...
						elif (timeSinceActive[i] > LOW_DEADTIME_THRESH):
							timeInPolicy[i] = MIN_ON
							print "low, increasing, has been " + str(timeSinceActive[i]) + " since active"
					...
					if (int(entry_array[i]) == 1):
						...
						if (time_left > 0):
							...
							timeInPolicy[i] = MIN_ON
							print "low, steady, has been " + str(timeSinceActive[i]) + " since active"
			...
			if (frequency_class == MID_LOW_FREQ):
				if (trend == INCREASING):
					...
					if (int(entry_array[i]) == 1):
						...
						if (time_left > 0):
							timeInPolicy[i] = time_left - 1
						elif (timeSinceActive[i] > MID_LOW_DEADTIME_THRESH):
							timeInPolicy[i] = MIN_ON
							print "mid low, increasing, has been " + str(timeSinceActive[i]) + " since active"
					...
					if (int(entry_array[i]) == 1):
						...
						if (time_left == 1):
							...
							timeInPolicy[i] = MIN_ON
							print "mid low, decreasing, has been " + str(timeSinceActive[i]) + " since active"
					...
					if (int(entry_array[i]) == 1):
						...
						if (time_left > 0):
							...
							timeInPolicy[i] = MIN_ON
							print "mid low, steady, has been " + str(timeSinceActive[i]) + " since active"
			...
			if (frequency_class == MID_HIGH_FREQ):
				if (trend == INCREASING):
					...
					if (int(entry_array[i]) == 1):
						...
						if (timeInPolicy[i] > 0):
							timeInPolicy[i] = timeInPolicy[i] - 1
						elif (timeSinceActive[i] > MID_HIGH_DEADTIME_THRESH):
							timeInPolicy[i] = MIN_ON
							print "mid high, increasing, has been " + str(timeSinceActive[i]) + " since active"
					...
					if (int(entry_array[i]) == 1):
						...
						if (timeInPolicy[i] > 0):
							...
							timeInPolicy[i] = MIN_ON
							print "mid high, decreasing, has been " + str(timeSinceActive[i]) + " since active"
					...
					if (int(entry_array[i]) == 1):
						...
						if (timeInPolicy[i] > 0):
							...
							timeInPolicy[i] = MIN_ON
							print "mid high, steady, has been " + str(timeSinceActive[i]) + " since active"
			...
			if (frequency_class == HIGH_FREQ):
				if (trend == INCREASING):
					...
					if (int(entry_array[i]) == 1):
						...
						if (timeInPolicy[i] > 0):
							timeInPolicy[i] = timeInPolicy[i] - 1
						elif (timeSinceActive[i] > HIGH_DEADTIME_THRESH):
							timeInPolicy[i] = MIN_ON
							print "high, increasing, has been " + str(timeSinceActive[i]) + " since active"
					...
					if (int(entry_array[i]) == 1):
						...
						if (timeInPolicy[i] > 0):
							...
							timeInPolicy[i] = MIN_ON
							print "high, decreasing, has been " + str(timeSinceActive[i]) + " since active"
					...
					if (int(entry_array[i]) == 1):
						...
						if (timeInPolicy[i] > 0):
							...
							timeInPolicy[i] = MIN_ON
							print "high, steady, has been " + str(timeSinceActive[i]) + " since active"
			...
			if (int(entry_array[i]) == 0 and time_left > 0 and timeHasBeenActive[i] >= MIN_ON):
...
# savename = "complicate/policies_hist5_first.txt"
# runExperiment(filename, hist_length, savename)

# filename = "experiment_data/second_week.txt"
# savename = "complicate/policies_hist5_second.txt"
# runExperiment(filename, hist_length, savename)

# filename = "experiment_data/third_week.txt"
# savename = "complicate/policies_hist5_third.txt"
# runExperiment(filename, hist_length, savename)
...
# filename = "experiment_data/first_week.txt"
# savename = "complicate/policies_hist10_first.txt"
# runExperiment(filename, hist_length, savename)

# filename = "experiment_data/second_week.txt"
# savename = "complicate/policies_hist10_second.txt"
# runExperiment(filename, hist_length, savename)

# filename = "experiment_data/third_week.txt"
# savename = "complicate/policies_hist10_third.txt"
# runExperiment(filename, hist_length, savename)
...
filename = "experiment_data/first_week.txt"
savename = "complicate_orig/policies_hist15_first.txt"
runExperiment(filename, hist_length, savename)

filename = "experiment_data/second_week.txt"
savename = "complicate_orig/policies_hist15_second.txt"
runExperiment(filename, hist_length, savename)

filename = "experiment_data/third_week.txt"
savename = "complicate_orig/policies_hist15_third.txt"
runExperiment(filename, hist_length, savename)
...
# filename = "experiment_data/first_week.txt"
# savename = "complicate/policies_hist20_first.txt"
# runExperiment(filename, hist_length, savename)

# filename = "experiment_data/second_week.txt"
# savename = "complicate/policies_hist20_second.txt"
# runExperiment(filename, hist_length, savename)

# filename = "experiment_data/third_week.txt"
# savename = "complicate/policies_hist20_third.txt"
# runExperiment(filename, hist_length, savename)
...
# filename = "experiment_data/first_week.txt"
# savename = "complicate/policies_hist30_first.txt"
# runExperiment(filename, hist_length, savename)

# filename = "experiment_data/second_week.txt"
# savename = "complicate/policies_hist30_second.txt"
# runExperiment(filename, hist_length, savename)

# filename = "experiment_data/third_week.txt"
# savename = "complicate/policies_hist30_third.txt"
# runExperiment(filename, hist_length, savename)
